...
interface BlogHeroProps {
  ...
  background?: string;
  coverImage?: string; // New prop for blog cover image
  useCoverImage?: boolean; // Flag to use cover image instead of default background
}
...
export default function BlogHero({
  ...
  const backgroundImage = useCoverImage && coverImage ? coverImage : background;

  return (
    <section
      className="relative flex lg:h-[53vh] h-[20%] flex-col md:flex-row items-center lg:justify-between px-6 sm:px-10 md:px-32 py-8 md:py-20 bg-cover bg-center bg-no-repeat text-white"
      style={{ backgroundImage: `url('${backgroundImage}')` }}
    >
      {/* Overlays - darker for better text readability */}
