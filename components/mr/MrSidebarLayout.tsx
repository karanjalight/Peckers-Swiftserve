    ...
    <SidebarProvider className="bg-white dark:bg-slate-900">
      ...
      <SidebarInset className="flex min-w-0 flex-1 flex-col bg-white dark:bg-slate-900">
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-3 dark:border-slate-700 dark:bg-slate-900 sm:gap-4 sm:px-4 lg:px-6">
          <SidebarTrigger className="-ml-1 size-8 shrink-0 bg-white text-slate-700 dark:text-slate-200 sm:size-9" />
          <Separator orientation="vertical" className="h-5 hidden sm:block dark:bg-slate-600" />
          <Link
            href="/mr"
            className="hidden shrink-0 bg-white sm:flex sm:items-center sm:gap-2 lg:hidden"
            aria-label="Home"
          >
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex min-w-0 items-center gap-2 rounded-full border border-slate-500 bg-slate-50 py-1.5 pl-2 pr-1.5 dark:border-slate-600 dark:bg-slate-800">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-800 text-xs font-medium text-white dark:bg-slate-500">
                {user.name.charAt(0).toUpperCase()}
              </div>
              <span className="hidden max-w-[100px] mr-3 truncate text-sm font-medium text-slate-800 dark:text-slate-200 md:inline lg:max-w-[120px]">
                {user.name}
              </span>
