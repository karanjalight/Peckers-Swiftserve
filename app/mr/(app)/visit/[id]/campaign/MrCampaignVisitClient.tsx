...
export function MrCampaignVisitClient({
  ...
  const generateProductsTablePdf = async () => {
    const { jsPDF } = await import("jspdf");
    ...
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("Products – stock, impact & order", margin, y);
    y += 5;
    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100, 100, 100);
    doc.text(`Generated by Peckers Swiftserve | ${new Date().toLocaleDateString("en-GB", { dateStyle: "medium" })}`, margin, y);
    ...
    y += 4;
    doc.setFont("helvetica", "normal");
    doc.text(`Region: ${pharmacy.region} | Location: ${pharmacy.location || "—"}`, margin, y);
    y += 10;
    ...
    if (tableRows.length === 0) {
      doc.setFont("helvetica", "normal");
      doc.text("No products recorded. Complete the key products section above to see products here.", margin, y);
      y += 10;
    } else {
      const colWidths = [38, 22, 18, 28, 22, 18, 28, 32, 22, 18];
      const fontSize = 7;
      doc.setFontSize(fontSize);
      doc.setFont("helvetica", "bold");
      const headers = ["Product", "Price", "Stock", "Qty sold", "Price/pack", "Days OOS", "Days last", "Lost rev", "Order qty", "Bonus"];
      let x = margin;
      headers.forEach((h, i) => {
        doc.text(h, x, y);
        x += colWidths[i];
      ...
      for (const row of tableRows) {
        if (y > pageHeight - 20) {
          ...
          y = margin;
          doc.setFontSize(8);
          doc.text(`Generated by Peckers Swiftserve | ${pharmacy.name}`, margin, y);
          y += 8;
        }
        x = margin;
        const cells = [
          (row.productName ?? "").slice(0, 24),
          row.productPrice != null ? row.productPrice.toLocaleString() : "—",
          String(row.stock ?? "—"),
          row.qtyGood ? String(row.qtyGood) : "—",
          row.price != null ? row.price.toLocaleString() : "—",
          row.daysOos ? String(row.daysOos) : "—",
          row.daysLast ? String(row.daysLast) : "—",
          row.lostRevenue ? row.lostRevenue.toLocaleString() : "—",
          String(row.quantityOrdered ?? 0),
          String(row.bonusQuantity ?? 0),
        ];
        cells.forEach((val, i) => {
    ...
    doc.setFontSize(7);
    doc.setTextColor(100, 100, 100);
    doc.text("Generated by Peckers Swiftserve", margin, pageHeight - 8);
    doc.setTextColor(0, 0, 0);

    doc.save(`products-stock-impact-order-${pharmacy.name.replace(/\s+/g, "-")}-${new Date().toISOString().slice(0, 10)}.pdf`);
  };
      ...
      <Card className="rounded-2xl border-slate-200 dark:border-slate-800">
        ...
        <CardContent>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-end gap-2">
            ...
            <Button variant="outline" onClick={generateProductsTablePdf} className="gap-2 rounded-full">
