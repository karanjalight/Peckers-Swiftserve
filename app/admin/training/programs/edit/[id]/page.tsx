...
export default function EditTrainingProgramPage() {
  ...
  const fetchProgram = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        cohort_number: data.cohort_number?.toString() || "",
        total_price: data.total_price?.toString() || "",
        deposit_amount: data.deposit_amount?.toString() || "",
        balance_due_days: data.balance_due_days?.toString() || "14",
        start_date: data.start_date ? data.start_date.split("T")[0] : "",
        end_date: data.end_date ? data.end_date.split("T")[0] : "",
        enrollment_deadline: data.enrollment_deadline ? data.enrollment_deadline.split("T")[0] : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Failed to update training program");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <SidebarLayout title="Edit Training Program">
    ...
    <SidebarLayout title="Edit Training Program">
      <div className="p-4 space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Edit Training Program</h1>
            <p className="text-slate-600 text-sm">Update the training program details below</p>
          </div>
