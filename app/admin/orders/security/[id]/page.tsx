...
export default function SecurityRequestsPage() {
  ...
  const [filteredRequests, setFilteredRequests] = useState<SecurityRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Filters
  const [searchTerm, setSearchTerm] = useState("");
  const [filterReason, setFilterReason] = useState<SecurityReason | "all">("all");
  ...
  useEffect(() => {
    ...
    // Reason filter
    if (filterReason !== "all") {
      filtered = filtered.filter((req) => req.reason === filterReason);
    }
  ...
  }, [requests, searchTerm, filterReason, filterStatus, sortBy]);
  ...
  const getStatusColor = (req: SecurityRequest) => {
  ...
  // Get status text
  const getStatusText = (req: SecurityRequest) => {
    ...
    <SidebarLayout title="Security Requests">
      <div className="flex-1 space-y-6 p-2 pt-6 bg-white min-h-screen">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Security Requests</h1>
            <p className="text-slate-600 mt-1">Manage security service requests</p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFilterReason(
                    (e.target.value as SecurityReason | "all") || "all"
                  )
                }
                className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                <option value="all">All Reasons</option>
                <option value="travel_vacation">{REASONS.travel_vacation}</option>
                <option value="night_shift">{REASONS.night_shift}</option>
                <option value="house_help_exit">{REASONS.house_help_exit}</option>
                <option value="construction_period">{REASONS.construction_period}</option>
                <option value="high_risk_period">{REASONS.high_risk_period}</option>
                ...
                <tbody>
                  {filteredRequests.map((request) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-600">
                        {request.dog_option
                          ? DOG_OPTIONS[request.dog_option]
                          : "—"}
                      </td>
                      <td className="px-6 py-4 text-slate-600">
                        {request.reason ? REASONS[request.reason] : "—"}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        ...
                        <Link
                            href={`/admin/orders/security/${request.id}`}
